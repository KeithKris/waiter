  ...
  while True:
    ...
    if not result['order_complete']:
        ...
        if not result['item_number']:
            print ("invalid order")
            messages.append({"role": "system", "content": "The previous response was invalid. item_number cannot be null when order_complete is false. There is enough information to complete the order in this prompt. Complete it correctly."})
            response = chat.get_response(messages)
            messages.append({"role": "assistant", "content": response})
            print(f"Llama: {response}")
        else:
